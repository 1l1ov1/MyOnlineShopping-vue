...
const changeStoreStatus = (row) => {
  updateRef.value = row
  const msg = row.accountStatus === 0 ? '开店中' : '关店中'
...
const sortTable = ({ column, prop, order }) => {
  ...
  if (order === 'ascending') {
    storeList.value.sort((a, b) => (compare(a[prop], b[prop]) ? 1 : -1))
  } else if (order === 'descending') {
    storeList.value.reverse()
  }
}
...
<template>
  <pageContainer title="我的商店">
    <template #extra>
      <!-- <el-button @click="onAddStore" type="primary" ref="dialog">添加商店</el-button> -->
      <el-button @click="batchDeleteStore()" type="primary">商店注销</el-button>
    </template>
    <!-- 行内表单 -->
    <el-form inline>
